	...
	public class ColorMixerWidget : Widget
	{
		...
		public void SetColorLimits(float minSaturation, float maxSaturation, float minValue, float maxValue, float? newHue = null)
		{
			...
			unsafe
			{
				...
				{
					var c = (int*)cc;
					for (var v = 0; v < 256; v++) // value increases horizontally left to right
					for (var s = 0; s < 256; s++) // saturation decreases vertically top to bottom
						(*(c + s * 256 + v)) = Color.FromAhsv(newHue ?? H, 1 - s / 255f, v / 255f).ToArgb();
				}
			}
